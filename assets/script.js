// Вопросы по Основам управления проектами (все вопросы из MD файла)
const PM_QUESTIONS = [
  ...
  {q: 'Оценки продолжительности задания О=3 дня, Р=7 дней, М=4 дня. Каково стандартное отклонение этого задания?', opts: ['5/6 дня', '1/2 дня', '2,5 дня', '2/3 дня'], a: 3, explain: 'Стандартное отклонение = (P - O) / 6 = (7 - 3) / 6 = 2/3 дня'},
  {q: 'В проектной организации команда проекта:', opts: ['Не несет ответственности за проект', 'Все ответы правильные', 'Отчитывается перед несколькими руководителями', 'Отчитывается перед функциональным менеджером'], a: 4, explain: 'В проектной организации команда проекта никогда не будет иметь собственного отдела, в который она вернется после завершения проекта'},
  {q: 'Что из перечисленного должен содержать Устав проекта?', opts: ['План управления коммуникациями', 'Деловые потребности проекта', 'Оценки заданий', 'План управления рисками'], a: 1, explain: 'Устав проекта должен содержать деловые потребности проекта'},
  {q: 'Активы организационного процесса включают следующее, кроме:', opts: ['Процедуры финансового контроля', 'Требования к коммуникациям', 'Процессы и процедуры организации для проведения работ', 'Ситуация на рынке'], a: 3, explain: 'Ситуация на рынке не является активом организационного процесса'},
  {q: 'Проект по разработке программного обеспечения идет непросто. В проекте свыше 30 участников и они не могут согласовать цели проекта. Один из участников проекта считает, что проект позволит достичь улучшения 30%, второй надеется, что возможно достичь 50%. Менеджер проекта считает, что 10% - наиболее реалистичны. Каков наилучший порядок действий?', opts: ['Все ответы правильные', 'Ускорить проект в надежде, что в ходе исполнения поступит больше информации', 'Выполнить анализ осуществимости', 'Усреднить эти цифры и использовать среднее как цель'], a: 2, explain: 'Наилучший порядок действий — выполнить анализ осуществимости'},
  {q: 'Положительное значение CV означает:', opts: ['отставание от расписания', 'недорасход бюджета', 'перерасход бюджета', 'данная величина не относится к бюджету'], a: 1, explain: 'Положительное значение CV (Cost Variance) означает недорасход бюджета'},
  {q: 'Выберите формулу расчета ETC:', opts: ['BAC-EAC', 'EAC-AC', 'EV-PV', 'BAC/CPI'], a: 1, explain: 'ETC (Estimate to Complete) = EAC - AC'},
  {q: 'Система кода счетов в иерархической структуре работ (ИСР) позволяет персоналу проекта:', opts: ['Систематически оценивать стоимость элементов ИСР', 'Определять уровень на котором находятся отдельные элементы', 'Проводить реабилитацию проекта', 'Систематически оценивать длительность элементов ИСР'], a: 1, explain: 'Система кода счетов позволяет определять уровень элементов'},
  {q: 'Резерв задания определяется:', opts: ['С помощью метода Монте-Карло', 'Отставанием', 'Опережением', 'Временем ожидания между заданиями'], a: 4, explain: 'Резерв задания — это время, на которое может быть задержано задание до того, как оно замедлит критический путь'},
  {q: 'Если пессимистическая оценка продолжительности задания – 18 дней, а оптимистическая – 12 дней, чему равно стандартное отклонение задания?', opts: ['1 день', '1,3 дня', '6 дней', '2 дня'], a: 0, explain: 'Стандартное отклонение = (P - O) / 6 = (18 - 12) / 6 = 1 день'},
  {q: 'Плановый объем обозначается как:', opts: ['EV', 'CV', 'AC', 'SV'], a: 4, explain: 'PV (Planned Value) — плановый объем'},
  {q: 'Отрицательное значение CV означает:', opts: ['данная величина не относится к бюджету', 'перерасход бюджета', 'отставание от расписания', 'опережение расписания'], a: 1, explain: 'Отрицательное значение CV означает перерасход бюджета'},
  {q: 'Прогноз до завершения обозначается как:', opts: ['VAC', 'SPI', 'CPI', 'ETC'], a: 3, explain: 'ETC (Estimate to Complete) — прогноз до завершения'},
  {q: 'Стрелочная диаграмма отличается от узловой диаграммы, так как стрелочная диаграмма:', opts: ['Имеет зависимости только типа «окончание-окончание»', 'Имеет четыре зависимости между заданиями', 'Может включать фиктивные операции', 'Может использовать PERT'], a: 2, explain: 'Стрелочная диаграмма может включать фиктивные операции'},
  {q: 'Какое высказывание наилучшим образом описывает взаимосвязь между стандартным отклонением и риском?', opts: ['Стандартное отклонение означает то, что оценка точная', 'Стандартное отклонение означает то, насколько неточна оценка', 'Стандартное отклонение означает то, что в оценку включена раздутая смета', 'Взаимосвязи нет'], a: 1, explain: 'Стандартное отклонение показывает насколько неточна оценка'},
  {q: 'Факторы внешней среды предприятия включают следующее, кроме:', opts: ['Погодные условия', 'Ситуация на рынке', 'Историческая информация', 'Организационная или корпоративная культура и структура'], a: 2, explain: 'Историческая информация является внутренним фактором, а не внешней средой'},
  {q: 'Все нижеприведенные характеристики являются признаками проекта, за исключением:', opts: ['Определение рисков проекта', 'Определенная дата начала и окончания', 'Временность', 'Повторяемость каждый месяц'], a: 3, explain: 'Повторяемость не является признаком проекта - проект временный и уникальный'},
  {q: 'Индекс выполнения сроков обозначается как:', opts: ['VAC', 'CPI', 'BAC', 'ETC'], a: 4, explain: 'SPI (Schedule Performance Index) — индекс выполнения сроков'},
  {q: 'Выберите формулу расчета CV:', opts: ['EV-AC', 'EV/AC', 'BAC/CPI', 'EV/PV'], a: 0, explain: 'CV (Cost Variance) = EV - AC'},
  {q: 'Фактическая стоимость обозначается как:', opts: ['AC', 'EV', 'PV', 'CV'], a: 0, explain: 'AC (Actual Cost) — фактическая стоимость'},
  {q: 'Одним из преимуществ матричной организации по сравнению с функциональной является:', opts: ['Более простая отчетность', 'Более простая структура проекта', 'Более чем один начальник у команд проекта', 'Более совершенный контроль менеджера проекта над ресурсами'], a: 3, explain: 'Матричная организация обеспечивает лучший контроль менеджера проекта'},
  {q: 'Выберите формулу расчета EAC:', opts: ['EAC-AC', 'BAC/CPI', 'EV/PV', 'EV-PV'], a: 1, explain: 'EAC = BAC / CPI'},
  {q: 'Отрицательное значение SV означает:', opts: ['перерасход бюджета', 'отставание от расписания', 'опережение расписания', 'недорасход бюджета'], a: 1, explain: 'Отрицательное значение SV означает отставание от расписания'}
];
